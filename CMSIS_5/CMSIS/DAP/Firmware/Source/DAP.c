...
static uint32_t DAP_Connect(const uint8_t *request, uint8_t *response) {
  ...
  uint32_t request_length = 1U;
  uint32_t response_length = 1U;
  ...
  switch (port) {
    ...
    case DAP_PORT_JTAG:
      ...
      break;
#endif
    case DAP_PORT_CONFIGURE_SWDIO_PIN:
      DAP_Data.debug_port = DAP_PORT_SWD;
      uint8_t swdio_pin = *(request + 1);
      uint8_t actual_swdio_pin = PORT_SWD_SETUP(swdio_pin);
      *(response + 1) = actual_swdio_pin;
      request_length = 2U;
      response_length = 2U;
      break;
    default:
  ...
  *response = (uint8_t)port;
  return ((request_length << 16) | response_length);
}
