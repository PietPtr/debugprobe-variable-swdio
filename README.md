# SWDIO pin selection modification

This version of debugprobe is modified such that a different GPIO pin can be used for the SWDIO line on SWD. This allows flashing and debugging of several connected RPi Picos. Intended to run on an RPi pico. SWCLK is still on GP2, SWDIO can be set using the set_swdio.py script.

```
mkdir build
cd build
cmake .. -DPICO_SDK_FETCH_FROM_GIT=on -DDEBUG_ON_PICO=ON
make
```
